...
fn test_fails_when_addr_space_is_reserved() {
    ...
    assert_eq!(
        addr_result.unwrap_err(),
        AddressError::UseOfReservedAddressSpace(
            ADDR_RESERVED_SPACE,
            "stest1fejq2x3d79ukpkw06t7h6lndjuwzxdnj59npghsg43mh4".to_string()
        )
    );
}
...
fn test_fails_when_incorrect_charset() {
    ...
    assert_eq!(
        addr_result.unwrap_err(),
        AddressError::Bech32Error(bech32::Error::InvalidChar('i'))
    )
}
...
fn test_fails_when_unsupported_network() {
    ...
    assert_eq!(
        addr_result.unwrap_err(),
        AddressError::WrongNetwork("sut".to_string(), "stest".to_string())
    );
}
